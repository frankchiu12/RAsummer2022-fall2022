...
for date, statement in date_to_label_to_statement.items():
    ...
    if 'C' not in statement:
        ...
        C = statement['C']
    statement_list = [A, B, C]
    vectorizer = TfidfVectorizer(lowercase = True, ngram_range = (1,1), use_idf = True) 
    X = vectorizer.fit_transform(statement_list)
    features = vectorizer.get_feature_names_out()
    df_tfidfvect = pd.DataFrame(data = X.toarray(), index = ['A','B','C'], columns = features)

    if date not in date_to_vector:
        date_to_vector[date] = df_tfidfvect
...
with open('/Users/franksi-unchiu/Desktop/cs200python/RAsummer2022/tf_idf_cosine_statements_internal.csv', 'w') as f:
    writer = csv.writer(f)
